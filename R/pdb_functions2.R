...
#'@param RK.cleavage If TRUE, will make sure that the cleavage sites alternate between R and K.
#'@param files.list If uploading a list of files directly instead of the directory, list the files with the appropriate paths.
#'@author Emma Gail
#'@export
...
rbd.makeSeqHitList <- function(fasta_file, experiment_directory = NULL,
  ...
  if(is.null(files.list)){
    ...
    if(is.null(file_format)){
      warning('Error: no file format selected. Either input your list')
      return(NULL)
    }
  ...
  }  else {#end if(is.null(files.list)){

    list_of_files <- files.list

  } #end else to  if(is.null(files.list)){
  ...
  for(file_name in list_of_files){
    ...
    file_format <- strsplit(file_name1,'\\.')[[1]]
    file_format <- file_format[length(file_format)]
